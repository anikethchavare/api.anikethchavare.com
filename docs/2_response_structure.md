...
| **`message`** | `String`          | A human-readable message for successful requests, or a structured error object for failures.                    |
| **`data`** | `Object`          | The primary payload of the response (can be empty if no payload is returned).                                   |
| **`meta`** | `Object`          | Contextual metadata including rate limit thresholds and other information. |
