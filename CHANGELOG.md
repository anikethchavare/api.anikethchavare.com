...
### Changed
- **Dependency Changes:** Added `ably` and `pymongo`. Removed `psycopg-pool` and `psycopg[binary]`. Updated `numpy` and `ably`.
...
- **math:** Added the following sub-utility namespaces: `/trigonometry`, `/statistics`, `/algebra`, `/arithmetic`, `/complex`, and `/geometry`.
...
- **math:** Added 7 endpoints (`/circumference`, `/area-of-sector`, `/arc-length`, `/distance`, `/area`, `/volume`, `/surface-area`) under the `/geometry` sub-utility namespace.
- **entertainment:** Added 6 endpoints (`/jokes`, `/fact`, `/bored`, `/guess-gender`, `/guess-age`, `/guess-nation`).
- **math:** Created tests for the following sub-utility namespace endpoints: `/trigonometry`, `/statistics`, `/algebra`, `/arithmetic`, `/complex`, and `/geometry`.
